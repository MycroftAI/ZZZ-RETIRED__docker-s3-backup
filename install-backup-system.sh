...
docker run -d -e access_key='${access_key}' -e secret_key='${secret_key}' -e s3_bucket='${s3_bucket}' -e s3_bucket_path=${HOSTNAME} --privileged -v /:/volume-mount/ '${docker_image}' """ > /docker-s3-tar-incremental.sh
...
echo "5 * * * * /docker-s3-tar-incremental.sh" >> mycron
