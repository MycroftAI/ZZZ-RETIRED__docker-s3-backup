...
function backup_operation {
	...
	then
		...
		$(python mailer.py)
		 curl -H "Content-Type: application/json" -X POST -d '{"hostname":"'${s3_bucket_path}'", "desc":"description coming soon!", "time": "'${timestamp}'", "success":"0"}' https://backups.mycroft.team/nodes
	else
		...
		curl -H "Content-Type: application/json" -X POST -d '{"hostname":"'${s3_bucket_path}'", "desc":"description coming soon!", "time": "'${timestamp}'", "success":"1"}' https://backups.mycroft.team/nodes
	fi
