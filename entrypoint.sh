...
function backup_operation {
	...
	then
		...
		$(python mailer.py)
		 curl -H "Content-Type: application/json" -X POST -d '{"hostname":"'${s3_bucket_path}'", "desc":"description coming soon!", "time": "'${timestamp}'", "success":"0"}' http://backups.mycroft.team:9898/nodes
	else
		...
		curl -H "Content-Type: application/json" -X POST -d '{"hostname":"'${s3_bucket_path}'", "desc":"description coming soon!", "time": "'${timestamp}'", "success":"1"}' http://backups.mycroft.team:9898/nodes
	fi
